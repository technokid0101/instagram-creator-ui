...
export default function DashboardPage() {
  ...
  async function generate() {
    if (!form.reelTopic) {
      setError("Reel topic is required.");
      return;
    }
        ...
        <form className="panel composer" onSubmit={(event) => event.preventDefault()}>
          ...
          <section className="fields">
            <Field label="Reel topic">
              <input value={form.reelTopic} onChange={(event) => update("reelTopic", event.target.value)} placeholder="e.g., quick paneer sandwich" />
            </Field>
            ...
            <Field label="Video description">
              <textarea
                ...
                placeholder="e.g., Shows ingredients, quick cooking steps, final plate"
              />
            </Field>
            ...
            <Field label="Goal">
              <input value={form.goal} onChange={(event) => update("goal", event.target.value)} placeholder="e.g., more saves and shares" />
            </Field>

            {mode === "hashtags" && (
              <Field label="Location or community">
                <input value={form.locationOrCommunity} onChange={(event) => update("locationOrCommunity", event.target.value)} placeholder="e.g., India" />
              </Field>
            )}

            {mode === "comment-replies" && (
              <Field label="Audience comment">
                <input value={form.comment} onChange={(event) => update("comment", event.target.value)} placeholder="e.g., This looks so good. Please share the recipe!" />
              </Field>
            )}
            ...
            {mode === "content-calendar" && (
              <div className="two-col">
                <Field label="Posting frequency">
                  <input value={form.postingFrequency} onChange={(event) => update("postingFrequency", event.target.value)} placeholder="e.g., 1 reel per day" />
                </Field>
                <Field label="Days">
            ...
            <details>
              ...
              <Field label="Creator name">
                <input value={form.creatorName} onChange={(event) => update("creatorName", event.target.value)} placeholder="e.g., Sujata" />
              </Field>
              <Field label="Niche">
                <textarea rows={2} value={form.niche} onChange={(event) => update("niche", event.target.value)} placeholder="e.g., food, comedy, and personal vlogs" />
              </Field>
              <Field label="Content pillars">
                <textarea rows={2} value={form.contentPillars} onChange={(event) => update("contentPillars", event.target.value)} placeholder="e.g., recipes, food reviews, funny daily moments, personal vlog clips" />
              </Field>
              <Field label="Creator style">
                <textarea rows={2} value={form.creatorStyle} onChange={(event) => update("creatorStyle", event.target.value)} placeholder="e.g., relatable, family-friendly, simple, funny" />
              </Field>
              <Field label="Target audience">
                <textarea rows={2} value={form.targetAudience} onChange={(event) => update("targetAudience", event.target.value)} placeholder="e.g., people who enjoy easy food ideas and relatable everyday content" />
              </Field>
            </details>
          </section>
